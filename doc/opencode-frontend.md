...
Attachments remain in the OpenCode session workspace until the session ends. Text files are stored as context directly. Formats such as PDF and DOCX are converted to Markdown when MarkItDown is available. The OpenCode Docker image installs `markitdown[pdf,docx]==0.1.7`; host development installs it optionally with `python3 -m pip install 'markitdown[pdf,docx]==0.1.7'`. Its absence does not prevent ordinary CQL chat or MCP use; a PDF/DOCX upload instead returns actionable installation guidance. `/compact` may retain summarized context while allowing the runner to purge original attachment files.
