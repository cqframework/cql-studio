...
- **`server/`** (`@cql-studio/server`) – Express and Node.js ESM backend, Prisma ORM, MCP tool orchestrator, Ollama & VSAC proxies, OIDC BFF session authentication, and the authenticated OpenCode gateway.
- **`opencode/`** (`@cql-studio/opencode`) – Private OpenCode runner process (workspace filesystem, `opencode serve`, MarkItDown). Published as `hlseven/quality-cql-studio-opencode`.
- **`ui/`** (`@cql-studio/ui`) – Angular 22 standalone frontend with CodeMirror 6, Bootstrap 5, and in-browser SQL on FHIR engine.
- **`docker/`** – Local Docker Compose stack providing PostgreSQL (CQL Studio DB & Authentik), Authentik (SSO/OIDC), HAPI FHIR R4 JPA server, and the OpenCode runner.
...
Start local backing infrastructure (PostgreSQL, Authentik, HAPI FHIR, CQL Tests Runner/Results) using the development compose file. Run the API, OpenCode runner, and UI from source on the host.
...
To run the **pre-built** UI, Server, and OpenCode images on top of that infrastructure:
...
Images pulled by the full stack: `hlseven/quality-cql-studio`, `hlseven/quality-cql-studio-server`, and `hlseven/quality-cql-studio-opencode`.
...
The UI start script loads `ui/.env`, the server loads `server/.env`, and the
OpenCode runner loads `opencode/.env`. These files are optional: when one is
absent, values exported by the parent shell are used. When a file is present,
variables declared in it take precedence while omitted variables still fall
back to the shell environment. To use a remote Ollama instance, update the UI
file, for example:
...
### 5. Start Server, Runner, and UI from Source

For local AI sessions with the development infrastructure stack, run the private runner alongside the server (or use `npm run docker:full:up` for pre-built images).

```bash
# Terminal 1: Start the backend API & MCP Server (runs in watch mode via tsx)
npm run start:server

# Terminal 2: Start the OpenCode runner
...
Published OpenCode runner images are available as `hlseven/quality-cql-studio-opencode` (multi-arch `latest` on `master`, plus branch-specific `*-linux-amd64` / `*-linux-arm64` tags). Build locally with:

```bash
docker build -f opencode/Dockerfile -t hlseven/quality-cql-studio-opencode:local .
```
...
| **OpenCode Runner** | `http://127.0.0.1:4097` | Private local AI runtime (`@cql-studio/opencode` / `hlseven/quality-cql-studio-opencode`); accessed through CQL Studio Server |
...
| `CQL_STUDIO_SERVER_SSO_ISSUER_URL` | Yes | `http://localhost:9000/application/o/cql-studio/` | OIDC SSO Issuer URL (browser-facing) |
| `CQL_STUDIO_SERVER_SSO_DISCOVERY_URL` | No | (same as issuer) | Internal issuer URL for Docker/compose discovery and token calls; keep issuer as localhost for the browser |
...
| `CQL_STUDIO_SERVER_OPENCODE_TOOL_BRIDGE_URL` | No | `http://127.0.0.1:3003/api/opencode/tool-bridge` | Gateway URL used by the runner's MCP subprocess (host-dev default; full compose overrides to the server service) |
...
OpenCode runner (`opencode/.env`) uses the `CQL_STUDIO_OPENCODE_*` prefix:

| Variable | Required | Default / Local Development Value | Description |
| --- | --- | --- | --- |
| `CQL_STUDIO_OPENCODE_NODE_ENV` | No | `development` | Node environment (`development` / `production`) |
| `CQL_STUDIO_OPENCODE_LOG_LEVEL` | No | `info` | Pino log level |
| `CQL_STUDIO_OPENCODE_RUNNER_PORT` | No | `4097` | Private runner HTTP listen port |
| `CQL_STUDIO_OPENCODE_RUNNER_TOKEN` | Production | Development-only shared token | Must match `CQL_STUDIO_SERVER_OPENCODE_RUNNER_TOKEN`; ≥32 bytes and non-default in production |
| `CQL_STUDIO_OPENCODE_INTERNAL_PORT` | No | `4096` | Loopback port for embedded `opencode serve` |
| `CQL_STUDIO_OPENCODE_SESSION_IDLE_MS` | No | `3600000` | Runner-side session idle expiration |
| `CQL_STUDIO_OPENCODE_CLEANUP_INTERVAL_MS` | No | `60000` | Runner-side orphan session cleanup interval |
| `CQL_STUDIO_OPENCODE_WORKSPACE_ROOT` | No | `./workspaces` (host) / `/workspaces` (image) | Ephemeral session filesystem root |
| `CQL_STUDIO_OPENCODE_RUNNER_REWRITE_LOCALHOST` | No | `false` (host) / `true` (Docker) | Rewrite Ollama `localhost`/`127.0.0.1` to `host.docker.internal` |
| `CQL_STUDIO_OPENCODE_PROVIDER_STALL_MS` | No | `180000` | Max wait for provider events before failing a prompt |
...
| `CQL_STUDIO_RUNNER_FHIR_BASE_URL` | `http://cql-studio-hapi-r4-data:8080/fhir` | FHIR URL used from inside the runner container |
